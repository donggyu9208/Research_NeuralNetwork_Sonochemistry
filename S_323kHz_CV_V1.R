...
## hidden = c(first layer, second layer)
## when one hidden layer
learning_rate <- c(0.05, 0.04, 0.03, 0.02, 0.01)
for (neuron in 1:100) { # number of neurons
  for (lr_i in 1:length(learning_rate)) { # learning rate index from 1 to 4
    ...
    for(i in 1:k) {
      ...
      net.data <- neuralnet(surfate_conc ~ frequency + temperature + power + air + Ar + Ar_O2 + He + O2, train_Data, 
                            hidden = c(h), err.fct = 'sse', act.fct = 'logistic', linear.output = FALSE, learningrate = curr_learningRate)
    ...
    if (min_cv_error > mean(cv_error_array)) {
      min_cv_error <- mean(cv_error_array)
      best_neuron <- neuron
      best_learning_rate <- learning_rate[r]
