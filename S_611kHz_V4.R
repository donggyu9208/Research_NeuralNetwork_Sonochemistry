...
temp_611_graph <- (
  ...
  + scale_y_continuous(name = "mean sulfate (mM)", breaks = 0:24 * 0.05, limits = c(0.6, 0.95))
...
power_611_graph <- (
  ...
  + geom_point(size = 2) 
  + geom_xspline(spline_shape=-0.4,size= 0.7)
  + scale_x_continuous(name = "Power (W)", breaks = 0:6 * 50, limits = c(20, 180))
  + scale_y_continuous(name = "mean sulfate (mM)", breaks = 0:10 * 0.2, limits = c(0.5, 1.3))
  #+ scale_color_manual(values = c("Predicted" = 'red','Experimental' = 'blue')) 
  + scale_shape_manual(values = c('Predicted' = 17, 'Experimental' = 16))
  + labs(color = "", shape = "")
)
