...
## At Frequency = 323 kHz, power = 13, gas = air, temperature = independent
input_frequency <- 323
input_power <- 13
input_air <- 1
input_Ar <- 0
...
predict1 <- NULL
for (i in 1:10) {
  input_temperature <- 10 * i
  input_data <- data.frame(input_frequency, input_temperature, input_power, input_air, input_Ar, input_Ar_O2, input_He, input_O2)
  predict_output_data <- neuralnet::compute(net.data, input_data)
  predict1[i] <- predict_output_data$net.result
}
predict1 <- predict1 * 10
predict1_data <- data.frame(temp = ((1 : 10) * 10), value = predict1)

predict1_graph <- (
  ggplot(predict1_data, aes(x = temp, 
                                   y = value))
  + geom_point(size = 2) 
  + geom_xspline(spline_shape=-0.4,size= 0.7)
  + scale_x_continuous(name = "Temperature (C)", breaks = 0:10 * 10, limits = c(0, 100))
  + scale_y_continuous(name = "Sulfate Concentration (mM)", breaks = 0:60 * 0.5, limits = c(-0.1, 1))
  #+ scale_color_manual(values = c("Predicted" = 'red','Experimental' = 'blue')) 
)

## At Frequency = 500 kHz, power = 50, gas = Ar, temperature = independent
input_frequency <- 500
input_power <- 50
input_air <- 1
input_Ar <- 0
...
for (i in 1:10) {
  ...
  input_data <- data.frame(input_frequency, input_temperature, input_power, input_air, input_Ar, input_Ar_O2, input_He, input_O2)
  predict_output_data <- neuralnet::compute(net.data, input_data)
  predict2[i] <- predict_output_data$net.result
}
predict2 <- predict2 * 10
predict2_data <- data.frame(temp = ((1 : 10) * 10), value = predict2)

predict2_graph <- (
  ggplot(predict2_data, aes(x = temp, 
                            y = value))
  + geom_point(size = 2) 
  + geom_xspline(spline_shape=-0.4,size= 0.7)
  + scale_x_continuous(name = "Temperature (C)", breaks = 0:10 * 10, limits = c(0, 100))
  + scale_y_continuous(name = "Sulfate Concentration (mM)", breaks = 0:60 * 0.5, limits = c(0, 1.2))
  #+ scale_color_manual(values = c("Predicted" = 'red','Experimental' = 'blue')) 
)
